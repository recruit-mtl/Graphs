...
internal class PieGraphView<T: Hashable, U: NumericType>: UIView {
    ...
    private var graph: PieGraph<T, U>? {
        didSet {
            self.config.pieColors = DefaultColorType.pieColors(count: graph?.units.count ?? 0)
    ...
    init(frame: CGRect, graph: PieGraph<T, U>?) {
        ...
        self.config = PieGraphViewConfig(pieColors: DefaultColorType.pieColors(count: graph?.units.count ?? 0))
  ...
  required init?(coder aDecoder: NSCoder) {
    ...
    override func draw(_ rect: CGRect) {
        ...
        func convert<S: NumericType>(s: S, arr: [S], f: (S) -> S) -> [S] {
            ...
            case let .some(h, t):
              let d:S = f(h) + s
              return [d] + convert(s: h + s, arr:t, f: f)
            case .none:
              return []
            }
        }
        
        let colors = self.config.pieColors ?? DefaultColorType.pieColors(count: graph.units.count)
        ...
        percentages.enumerated().forEach { (index, f) in
            ...
            if self.config.isDounut {
              context?.addArc(center: CGPoint(x: x, y: y), radius: radius * 0.5, startAngle: CGFloat(endAngle), endAngle: CGFloat(startAngle), clockwise: true)
            }
